...
dxil_create_validator(const void *ctx)
{
   ...
   /* The compiler can emit DXIL without the external validator. Xbox retail
    * does not provide DXIL.dll to packaged applications. */
   val->dxil_mod = load_dxil_mod();
   if (!val->dxil_mod) {
#ifdef _XBOX_UWP
      static bool reported_missing_validator;
      if (!reported_missing_validator) {
         debug_printf("DXIL: external validator unavailable; validation disabled\n");
         reported_missing_validator = true;
      }
#else
    ...
    */
#ifdef _XBOX_UWP
   val->dxcompiler_mod = LoadPackagedLibrary(L"dxcompiler.dll", 0);
#else
